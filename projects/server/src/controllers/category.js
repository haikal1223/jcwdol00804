...
module.exports = {
      ...
      if (checkName.length) {
        ...
        db.query(
          ...
          (error, results, fields) => {
            ...
            return res.status(200).send({
              success: true,
              message: `Name changed into "${name}"`,
            });
          }
        );
      }
  ...
  restoreCategory: async (req, res) => {
    const { id } = req.params;
    db.query(
      `UPDATE category SET is_delete=0 WHERE id= ?`,
      [id],
      (error, results) => {
        if (error) {
          return res.status(500).send({
            success: false,
            message: "Error restore category",
          });
        }
        return res.status(200).send({
          success: true,
  ...
  getCategoryDetail: async (req, res) => {
    try {
      const { id } = req.params;
      db.query(`SELECT * FROM category WHERE id= ?`, [id], (error, results) => {
        if (error) {
          return res.status(500).send({
            success: false,
            message: error,
